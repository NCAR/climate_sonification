...
export function getClosestCity(lat: number, lon: number): string
{
  ...
  let thislat, thislon, dist, xdiff, ydiff, lon1, lon2;
  let closestcity = '';
  for (let i = 0; i < cities.length; i++)
  {
    const city = cities[i];
    if (!city) continue;
    thislat = city.latitude;
    thislon = city.longitude;
    if (lon > 0 && thislon < 0)
    {
      lon1 = thislon;
      lon2 = thislon + 360;
...
export function getInfo(city: string): City {
  ...
  for (let i = 0; i < cities.length; i++)
  {
    const temp_city = cities[i];
    if (!temp_city) continue;
    if (temp_city.city === city)
    {
      return cities[i] as City;
    }
  }
