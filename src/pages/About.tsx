...
function About({ navigation, route }) {
  ...
  const pageBottom = p.pageBottom ?? window.innerHeight;
  const pageRight = p.pageRight ?? window.innerWidth;

  const { titleTextStyle, questionTextStyle, answerTextStyle } = useMemo(() => {
    const smallFontSize = Math.floor(pageRight / 200 + pageBottom / 100);
    const microFontSize = smallFontSize - 2;
    const largeFontSize = smallFontSize + 4;
